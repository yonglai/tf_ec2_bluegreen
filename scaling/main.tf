resource "aws_cloudwatch_metric_alarm" "alarm_down" {
  count               = var.num_asg
  alarm_name          = format("%s-down%d", var.name, count.index)
...
resource "aws_cloudwatch_metric_alarm" "alarm_up" {
  count               = var.num_asg
  alarm_name          = format("%s-up%d", var.name, count.index)
...
resource "aws_autoscaling_policy" "up" {
  count                  = var.num_asg
  name                   = format("%s-up%d", var.name, count.index)
...
resource "aws_autoscaling_policy" "down" {
  count                  = var.num_asg
  name                   = format("%s-down%d", var.name, count.index)
  autoscaling_group_name = var.autoscaling_group_names[count.index]
  adjustment_type        = var.adjustment_type
  policy_type            = var.policy_type
